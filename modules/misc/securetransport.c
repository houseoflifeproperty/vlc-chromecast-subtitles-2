...
static int st_validateServerCertificate (vlc_tls_t *session, const char *hostname) {
    ...
    /* Check if this is a local network connection (mDNS .local or private IP) */
    bool is_local = false;
    if (hostname) {
        /* Check for mDNS .local suffix (Chromecast, AirPlay, etc.) */
        size_t len = strlen(hostname);
        if (len > 6 && strcmp(hostname + len - 6, ".local") == 0)
            is_local = true;
        else if (len > 7 && strcmp(hostname + len - 7, ".local.") == 0)
            is_local = true;
        /* Check for private IP ranges - must be valid IPv4 format */
        else {
            unsigned int a, b, c, d;
            char extra;
            /* sscanf with %c at end ensures no trailing characters (like .evil.com) */
            if (sscanf(hostname, "%u.%u.%u.%u%c", &a, &b, &c, &d, &extra) == 4) {
                /* Valid IPv4 format - check if private range */
                if (a == 10)  /* 10.0.0.0/8 */
                    is_local = true;
                else if (a == 192 && b == 168)  /* 192.168.0.0/16 */
                    is_local = true;
                else if (a == 172 && b >= 16 && b <= 31)  /* 172.16.0.0/12 */
                    is_local = true;
                else if (a == 127)  /* 127.0.0.0/8 loopback */
                    is_local = true;
            }
        }
    }
    ...
    if (is_local) {
        /* Auto-accept certificates for local network connections */
        msg_Warn(sys->obj, "Auto-accepting certificate for local host %s", hostname);
        ...
        /* Non-local connection with untrusted certificate - reject it */
        msg_Err(sys->obj, "Rejecting untrusted certificate for non-local host %s", hostname);
        result = -1;
        goto out;
    }
